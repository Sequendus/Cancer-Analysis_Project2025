...
Run setup script and load data. We select only the first 31 rows of the data as the rest are RNA profiles:
...
if (!file.exists("../data/data_clean.csv")) {
  ...
  message("data_clean.csv already exists — not overwriting")
}
glimpse(data_clean)
```
